...
def compute_kkt_error(ss, x, y):
    ...
    e = g - np.fmax(ss.D[0], np.fmin(g, ss.D[1]))
    # Constraint violation, ‖g(x) - Π(g(x))‖
    constr_violation = la.norm(e, ord=np.inf)
    # Complementary slackness
    complementarity = la.norm(y * e, ord=np.inf)
...
build_config = (
    ...
    .with_build_directory(".opengen_cache")
...
mng = og.tcp.OptimizerTcpManager(".opengen_cache/the_optimizer")
mng.start()
