...
build_dir="$PWD/../../../build/examples/benchmarks/Release"
...
run () { pushd "$output"; set -ex;
    for i in {$delta..60..$delta}; do
        /usr/bin/time -f 'max_memory: %M' "$build_dir/benchmark-mpc-driver" $problem $i $formulation method="$solver" num_sim=30 "$@"
    done
    popd
}
mkdir -p "$output"
