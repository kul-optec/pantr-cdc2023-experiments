...
void print_results(std::ostream &os, const BenchmarkResults &results) {
    ...
    os << '\n'
       ...
       << "problem: " << results.problem.path.filename().c_str() << " "
       << results.problem.horizon << " (from "
...
void write_results(std::ostream &os, const BenchmarkResults &results) {
    ...
    for (const auto &[key, value] : results.extra)
        std::visit([&, k{std::string_view{key}}](
                   ...
                   value);
    dict_elem(os, "problem", results.problem.name);
    dict_elem(os, "nvar", results.problem.problem.get_n());
    dict_elem(os, "ncon", results.problem.problem.get_m());
    dict_elem(os, "path", results.problem.full_path);
    dict_elem(os, "horizon", results.problem.horizon);
    dict_elem(os, "second_order", results.problem.second_order);
    dict_elem(os, "formulation", results.problem.formulation);
    os << "}\n";
}

struct MPCBenchmarkResults {
    USING_ALPAQA_CONFIG(alpaqa::DefaultConfig);
    static constexpr real_t NaN = alpaqa::NaN<config_t>;
...
void print_results(std::ostream &os, const MPCBenchmarkResults &results) {
    USING_ALPAQA_CONFIG(BenchmarkResults::config_t);
    using alpaqa::float_to_str;
    auto time_s = results.runtimes.cwiseAbs().sum();
    os << '\n'
       << results.evals << '\n'
       << "solver:  " << results.solver << '\n'
       << "problem: " << results.problem.path.filename().c_str() << " "
...
void write_results(std::ostream &os, const MPCBenchmarkResults &results) {
    os << "from numpy import nan, inf\n"
          "import numpy as np\n"
          "__all__ = ['results']\n"
          "results = {\n";
    dict_elem(os, "opts", results.options);
    dict_elem(os, "time_utc_ms", results.timestamp);
    dict_elem(os, "runtimes", results.runtimes);
    dict_elem(os, "states", results.states);
    dict_elem(os, "inputs", results.inputs);
    dict_elem(os, "solver", results.solver);
    write_evaluations(os, results.evals);
    for (const auto &[key, value] : results.extra)
        std::visit([&, k{std::string_view{key}}](
...
struct OCPMPCBenchmarkResults {
    USING_ALPAQA_CONFIG(alpaqa::DefaultConfig);
    static constexpr real_t NaN = alpaqa::NaN<config_t>;

    BenchmarkControlProblem &problem;
    alpaqa::OCPEvalCounter evals;
    std::string solver;
    vec runtimes;
    mat states;
    mat inputs;
...
void print_results(std::ostream &os, const OCPMPCBenchmarkResults &results) {
    USING_ALPAQA_CONFIG(BenchmarkResults::config_t);
    using alpaqa::float_to_str;
    auto time_s = results.runtimes.cwiseAbs().sum();
    os << '\n'
       ...
       << results.problem.horizon << " (from "
       << results.problem.path.parent_path() << ")" << '\n'
       << "num var: " << results.problem.n << '\n'
       << "num con: " << results.problem.m << '\n'
       << "time: " << float_to_str(time_s, 3) << " s\n"
       << std::endl;
    for (const auto &[key, value] : results.extra) {
        auto print = detail::overloaded{
...
void write_results(std::ostream &os, const OCPMPCBenchmarkResults &results) {
    os << "from numpy import nan, inf\n"
          "import numpy as np\n"
          "__all__ = ['results']\n"
          "results = {\n";
    ...
    for (const auto &[key, value] : results.extra)
        std::visit([&, k{std::string_view{key}}](
                   ...
                   value);
    dict_elem(os, "problem", results.problem.name);
    dict_elem(os, "nvar", results.problem.n);
    dict_elem(os, "ncon", results.problem.m);
    dict_elem(os, "path", results.problem.full_path);
    dict_elem(os, "horizon", results.problem.horizon);
    dict_elem(os, "second_order", false);
    dict_elem(os, "formulation", results.problem.formulation);
    os << "}\n";
}
